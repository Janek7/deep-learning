...
parser.add_argument("--final_avg_pooling", default=True, type=bool, help="Flag to use average pooling instead of dense layers.")
...
def main(args: argparse.Namespace) -> None:
    ...
    # TODO: try approaches from slide 3 and 11 from slide set 2 about CNNs + regularization methods
    ...
    for i in range(args.conv_layers):
        model.add(tf.keras.layers.Conv2D(filters=args.filters*max_pool_mul, kernel_size=args.kernel_size, strides=args.stride,
                                         padding=args.padding, activation=tf.nn.relu))
    ...
    if not args.final_avg_pooling:
        ...
        for i in range(args.dense_layers):
            model.add(tf.keras.layers.Dense(args.dense_size, activation=tf.nn.relu))
