...
class Model(tf.Module):
    ...
    def train_epoch(self, dataset: MNIST.Dataset) -> None:
        for i, batch in enumerate(dataset.batches(self._args.batch_size)):
            ...
            #print(f"{i + 1}. Batch".center(50, '-'))
            if batch["labels"].shape != (self._args.batch_size, 10):
                batch["labels"] = tf.keras.utils.to_categorical(batch["labels"])
                if batch["labels"].shape != (self._args.batch_size, 10):
                    print("ERROR")
                    continue
                # print(batch["labels"])
            # loss = tf.losses.CategoricalCrossentropy(reduction='none')(batch["labels"], output)

            # 1) W2, B2 -> output layer

            # COMPUTE DERIVATIVES
            # print(f"Shape Output: {output.shape}, Shape Labels: {batch['labels'].shape}")
            # compute derivative of loss and y_in (logits of output without softmax application)
            derivative_loss_yin = tf.math.subtract(output, batch["labels"])
            derivative_loss_yin_averaged = tf.reduce_mean(derivative_loss_yin, axis=0)
            #print(derivative_loss_yin)
            # print(f"Shape of derivative of loss w.r.t output logits ('blue circle'): {derivative_loss_yin.shape}")

            # COMPUTE GRADIENTS
            # gradient computation of W2
            derivative_loss_w2 = tf.einsum("ai,aj->aij", hidden, derivative_loss_yin)

            # print(f"Shape outer product: {hidden.shape} -> hidden * {derivative_loss_yin.shape} -> loss derivative = {derivative_loss_w2.shape}")
            # average gradients over batches -> reduce from (batchsize, 100, 10) to (100, 10)
            gradients_w2 = tf.reduce_mean(derivative_loss_w2, axis=0)
            # print(f"Shape of averaged gradients: {gradients.shape}")

            # 1) W1, B1 -> hidden layer
            derivative_hidden_activation_function = 1 - tf.math.square(hidden)
            s2 = (derivative_loss_yin @ tf.transpose(self._W2)) @ derivative_hidden_activation_function
            s2_averaged = tf.reduce_mean(s2, axis=0)
            gradients_w1 = tf.einsum("ai,aj->aij", input, s2_averaged)
            ...
            # print(f"Shape of W2: {self._W2.shape}")
            ...
            self._b1.assign_sub(self._args.learning_rate * s2_averaged)
